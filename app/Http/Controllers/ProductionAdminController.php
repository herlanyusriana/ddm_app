...
class ProductionAdminController extends Controller
{
    ...
    private function productionPageData(Request $request, string $type): array
    {
        ...
        $productionMonth = preg_match('/^\d{4}-\d{2}$/', (string) $request->query('production_month'))
            ...
            : substr($date, 0, 7);
        $selectedOperatorIds = $this->operatorFilterIdsFromRequest($request);
        $hourlyReport = $historyProcess
            ? $this->productionHourlyReport($date, $shift, $historyProcess, $historyPeriod, $productionMonth, $selectedOperatorIds)
    ...
    private function operatorFilterIdsFromRequest(Request $request): array
    {
        return $this->validOperatorFilterIds([
            ...((array) $request->query('operator_ids', [])),
            $request->query('operator_id'),
        ]);
    }

    private function operatorFilterIdsFromValidated(array $validated): array
    {
        return $this->validOperatorFilterIds([
            ...((array) ($validated['operator_ids'] ?? [])),
            $validated['operator_id'] ?? null,
        ]);
    }

    private function validOperatorFilterIds(array $rawIds): array
    {
        $ids = collect($rawIds)
        ...
        return Operator::whereIn('id', $ids)->pluck('id')->map(fn ($id) => (int) $id)->all();
    }
    ...
    public function productionHourlyExport(Request $request): Response
    {
        $validated = $request->validate([
            ...
            'operator_ids.*' => ['nullable', 'exists:operators,id'],
        ]);
        ...
        $report = $this->productionHourlyReport(
            ...
            $historyPeriod,
            $productionMonth,
            $this->operatorFilterIdsFromValidated($validated),
        );
    ...
    public function productionHourlyPrint(Request $request): View
    {
        ...
        $operatorFilterIds = $this->operatorFilterIdsFromValidated($validated);
